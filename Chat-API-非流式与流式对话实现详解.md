# Chat API 实现：非流式与流式对话

> 前两篇笔记讲了项目怎么搭、为什么这样分层。
> 这篇深入到最核心的 Chat API，看**非流式**和**流式**两种调用方式分别怎么实现。

***

## 目录

1. [两种模式的核心区别](#1-两种模式的核心区别)
2. [非流式实现：请求→等待→响应](#2-非流式实现请求等待响应)
3. [流式实现：请求→逐块推送→结束](#3-流式实现请求逐块推送结束)
4. [Service 层的差异](#4-service-层的差异)
5. [SSE 协议：流式的传输载体](#5-sse-协议流式的传输载体)
6. [reasoning_content 的处理](#6-reasoning_content-的处理)
7. [错误处理差异](#7-错误处理差异)
8. [完整代码对照](#8-完整代码对照)
...
def chat():
    ...
    try:
        ...
        return jsonify({"success": False, "error": {"code": "MIMO_API_ERROR", "message": f"AI 服务调用失败"}}), 500
```
...
> **核心要点：**
>
> 1. 非流式和流式的代码差异只有一行：`stream=True/False`
> 2. 流式的本质是**生成器**——`yield` 一次推一个 token
> 3. SSE 就是带 `text/event-stream` 头部的普通 HTTP 响应
> 4. 流式的错误无法返回普通 JSON，必须走 SSE 通道
> 5. `reasoning_content` 在两种模式下都处理，但提取位置不同（`message` vs `delta`）
