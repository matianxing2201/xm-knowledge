---
date: 2026-07-30
title: Tool Calling 大模型自主决策实现详解
tags:
  - AI
  - AI Agent
  - Tool Calling
...
和 Function Calling 那篇对比一下，Tool Calling 在实现上其实更简洁：

1. **Function Calling 用原生 SDK**，需要手动写 JSON Schema、手动分发函数、手动拼第二次请求，每一步都是自己干
2. **Tool Calling 用 LangChain 的 `@tool` + `bind_tools`**，docstring 直接当工具描述，省去了手写 JSON Schema 的麻烦
3. **核心决策逻辑不变**——都是先调模型判断要不要用工具，用了就执行再送回模型，没用就直接回答
4. **`response.tool_calls` 仍然是关键判断点**，非空就是模型要调工具，空就是直接回答
5. **流式输出用 `llm_with_tools.stream()`** 而不是 `llm.stream()`，这样模型能理解消息历史中的 ToolMessage

说白了，Function Calling 是底层原理，Tool Calling 是 LangChain 封装后的便捷写法。理解了前者，后者就是少了些样板代码而已。
